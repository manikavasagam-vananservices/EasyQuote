...
/**
 * Basic Price with Notarization + Mailing Hard copy(Transcription, Translation, Captioning, Typing)
 */
public class EasyQuoteL17 extends TestBase {
    ...
    public void runTranscriptionTest() {
        fileProcessing.setExcelFile(level17, service1);
        for (int i = 1; i <= fileProcessing.getRowUsed(); i++) {
            ...
            double additional = roundValues(fileProcessing.getFloatCellData(i, 5) + fileProcessing.getFloatCellData(i, 8));
            ...
            String AdditionalServicePrice = checkStatus(easyQuotePage.getAdditionalServicePriceValue(), additional, "Notarization + Mailing");
        ...
        fileProcessing.writeFileContent(level17);
    }

    @Test(priority = 1)
    public void runTranslationTest() {
        fileProcessing.setExcelFile(level17, service2);
        for (int i = 1; i <= fileProcessing.getRowUsed(); i++) {
            System.out.println("Source : " + fileProcessing.getCellData(i, 0));
            ...
            double additional =  roundValues(fileProcessing.getFloatCellData(i, 6) + fileProcessing.getFloatCellData(i, 9));
            double total =  roundValues(totalUnitCost + additional);
            double transactionFee =  roundValues(total * 0.05);
            double orderTotal =  roundValues(total + transactionFee);
